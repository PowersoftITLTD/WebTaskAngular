...
import { RecursiveTaskData } from 'src/app/interface/recursiveTaskData';
import { ApiService } from 'src/app/services/api/api.service';
import { CredentialService } from 'src/app/services/credential/credential.service';

@Component({
  selector: 'app-add-recursive-task-new',
  templateUrl: './add-recursive-task-new.component.html',
  styleUrls: ['./add-recursive-task-new.component.css']
})
export class AddRecursiveTaskNewComponent implements OnInit, OnDestroy {
  ...
  getSelectedTaskDetails(mkey: string) {
    ...
    return this.apiService.getSelectedTaskDetailsNew(mkey.toString(), token);
  }
...
setSubProjName(): void {
  ...
  fetchEmployeeName(): void {
    ...
    this.apiService.getEmpDetailsNew(token).subscribe(
      (response: any) => {
        // console.log("Employee data:", response[0]?.data);
        // const _data = data;
