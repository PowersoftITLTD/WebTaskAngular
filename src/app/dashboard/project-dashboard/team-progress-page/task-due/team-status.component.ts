  ...
  onFilterSelectionChange(option: string) {
    ...
    switch (option) {
      ...
      case 'Department Future':
          this.apiService.departmentFutureDetailsNew(CURRENT_EMP_MKEY.toString(), MKEY.toString(), token).subscribe((response) => {
            this.selectedDetails = this.filterDetails(response[0].data1, option)
            this.loading = false;
          });
          break;
      case 'Inter-Department To-day':
          this.apiService.interDepartmentTodayDetailsNew(CURRENT_EMP_MKEY.toString(), MKEY.toString(), token).subscribe((response) => {
            this.selectedDetails = this.filterDetails(response[0].data1, option)
            this.loading = false;
          });
          break;
      case 'Inter-Department Over-due':
          this.apiService.interDepartmentOverdueDetailsNew(CURRENT_EMP_MKEY.toString(), MKEY.toString(), token).subscribe((response) => {
            this.selectedDetails = this.filterDetails(response[0].data1, option)
            this.loading = false;
          });
          break;
      case 'Inter-Department Future':
          this.apiService.interDepartmentFutureDetailsNew(CURRENT_EMP_MKEY.toString(), MKEY.toString(), token).subscribe((response) => {
            this.selectedDetails = this.filterDetails(response[0].data1, option)
            this.loading = false;
          });
          break;
